...
function xmldb_tool_advancedreplace_upgrade($oldversion) {
    ...
    if ($oldversion < 2024101600) {
        ...
        $field = new xmldb_field('openzips', XMLDB_TYPE_INTEGER, '1',     null, XMLDB_NOTNULL, null, '1');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        $field = new xmldb_field('zipfilenames', XMLDB_TYPE_TEXT, null, null, XMLDB_NOTNULL, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
         $table->add_field('skipzipfilenames', XMLDB_TYPE_TEXT, null, null, XMLDB_NOTNULL, null, null);
         if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
         $table->add_field('zipmimetypes', XMLDB_TYPE_TEXT, null, null, XMLDB_NOTNULL, null, null);
         if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
         $table->add_field('skipzipmimetypes', XMLDB_TYPE_TEXT, null, null, XMLDB_NOTNULL, null, null);
         if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
