...
$url = new moodle_url('/admin/tool/advancedreplace/searchfiles.php');
$PAGE->set_url($url);

admin_externalpage_setup('tool_advancedreplace_searchfiles');
...
if (isset($delete)) {
    require_sesskey();
    $searchfiles = new \tool_advancedreplace\searchfiles($delete);
    $searchfiles->delete();
...
if (isset($id)) {
    $newurl = new moodle_url('/admin/tool/advancedreplace/searchfiles.php');
    $newurl->param('id', $id);
    ...
    $form = new \tool_advancedreplace\form\searchfiles($newurl->out(false), $customdata);
    if ($form->is_cancelled()) {
        redirect($url);
    } else if ($data = $form->get_data()) {
        if (empty($data->id)) {
            $searchfiles = new \tool_advancedreplace\searchfiles(0, $data);
            $searchfiles->create();
            $searchfiles->queue_task();
        ...
        // Display form.
        echo $OUTPUT->header();
        echo $OUTPUT->heading(get_string('searchfilespageheader', 'tool_advancedreplace'));
        if (isset($copy)) {
            // Load the original data without setting the persistent.
            $searchfiles = new \tool_advancedreplace\searchfiles($copy);
            $data = $searchfiles->copy_data();
            $form->set_data($data);
    ...
    $table = new \tool_advancedreplace\searchfiles_table('tool_advancedreplace');
