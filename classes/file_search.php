...
class file_search {
    ...
    * @return void
    */
    public function __construct(\tool_advancedreplace\files $record, string $output = '') {
        global $DB;
        ...
        $this->parameters = (object) [
            ...
            'zipmimetypes' => trim($record->get('zipmimetypes')),
            'skipzipfilenames' => trim($record->get('skipzipfilenames')),
            'skipzipmimetypes' => trim($record->get('skipzipmimetypes')),
        ];
    ...
    * @param array $csv Some columns to be output in the csv file.
    * @param string $content The actual bytes of the zip file.
    * @param string $parameters The searching criteria.
    * @param resource $stream The handle for the output file.
    * @return int $matchcount The number of matches found.
    */
    public static function unzip_content($csv, $content, $parameters, $stream): int {
        static $finfo = null;
        if ($finfo == null) {
        ...
        $zip = new \ZipArchive();
        $matchcount = 0;
        if (! empty ($parameters->zipfilenames)) {
            $namepattern = '%' . $parameters->zipfilenames . '%i';
        } else {
            $namepattern = '';
        }
        if (! empty ($parameters->zipmimetypes)) {
            $mimepattern = '%' . $parameters->zipmimetypes . '%i';
        } else {
            $mimepattern = '';
        }
        if ($zip->open($tmpfile) === true) {
            ...
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                // Filter by mimetype.
                if ( ! empty($mimepattern)) {
                    $filecontents = $zip->getFromIndex($i);
                    $mimetype = $finfo->buffer($filecontents);
                    if ( ! preg_match($mimepattern, $mimetype)) {
                        print "ignoring mimetype $mimetype\n";
                        continue;
                    }
                }
                
                $csv[self::CSV_INTERNAL] = $stat['name'];
                $matchcount += self::grep_content($csv, $filecontents, $parameters, $stream);
            }
            $zip->close();
        }
        ...
        switch ($filerecord->mimetype) {
            case 'application/zip.h5p':
                case 'application/zip':
                    // Todo: I openzips flag
                    $csv[self::CSV_STRATEGY] = 'zip';
                    $matchcount = self::unzip_content($csv, $file->get_content(), $this->parameters, $stream);
                    break;
                    default:
                    $csv[self::CSV_STRATEGY] = 'plain';
                    $csv[self::CSV_INTERNAL] = '';
                    $matchcount = self::grep_content($csv, $file->get_content(), $this->parameters, $stream);
                    break;
                }
                return $matchcount;
            }
            ...
            *
            * @param object $parameters The search criteria in an object.
            ...
            */
            public static function make_where_clause(object $parameters):array {
                $params = [];
                $paramnumber = 0;
                $whereclause = '';
                $and = ''; // For first one.
                ...
                if ( ! empty($parameters->components)) {
                    ...
                    $or = ''; // For first one.
                    foreach (explode(',', $parameters->components) as $specification) {
                ...
                if ( ! empty($parameters->mimetypes)) {
                    $whereclause .= $and . '( ';
                    $and = ' AND '; // For next one.
                    $or = ''; // For first time.
                    foreach (explode(',', $parameters->mimetypes) as $mimetype) {
                        $paramnumber ++;
                ...
                if ( ! empty($parameters->filenames)) {
                    $whereclause .= $and . '( ';
                    $and = ' AND '; // For next one.
                    $or = ''; // For first time.
                    foreach (explode(',', $parameters->filenames) as $filename) {
                        $paramnumber ++;
                ...
                if ( ! empty($parameters->skipcomponents)) {
                    foreach (explode(',', $parameters->skipcomponents) as $component) {
                        $paramnumber ++;
                ...
                if ( ! empty($parameters->skipmimetypes)) {
                    foreach (explode(',', $parameters->skipmimetypes) as $mimetype) {
                        $paramnumber ++;
                        $params["param{$paramnumber}"] = trim($mimetype);
                ...
                if ( ! empty($parameters->skipfilenames)) {
                    foreach (explode(',', $parameters->skipfilenames) as $filename) {
                        $paramnumber ++;
                        $params["param{$paramnumber}"] = trim($filename);
                ...
                if ( ! empty($parameters->skipareas)) {
                    foreach (explode(',', $parameters->skipareas) as $area) {
                        $paramnumber ++;
